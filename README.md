...
- **Analisi dei nomi dei branch, inclusi quelli chiusi** (per analisi da URL): i branch creati dagli strumenti di coding AI hanno prefissi riconoscibili (`claude/…`, `copilot/…`, `codex/…`, `cursor/…`, `devin/…`, ecc.). Vengono controllati sia i branch attivi sia quelli **chiusi/cancellati**, recuperati dalle pull request chiuse e dai messaggi dei merge commit; la loro presenza viene segnalata come anomalia e contribuisce alla stima complessiva.
- **Autori dei commit e delle righe**: gli autori dei commit vengono classificati (bot/agent AI come Claude, Copilot, Devin… — i bot di automazione tipo dependabot/renovate sono esclusi) con la quota di commit attribuibile ad AI; per i repository GitHub con `GITHUB_TOKEN` configurato viene eseguito anche il **git blame** (via API GraphQL) sui file più sospetti: se una quota di righe proviene da commit attribuiti ad AI, il file lo mostra tra le motivazioni e lo score non può scendere sotto quella quota.
